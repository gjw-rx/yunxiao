...
class ReadTool extends BaseTool {
	readonly schema: ToolSchema = {
		name: 'fs.read_file', description: 'read', parameters: {}, permissions: 'read', site: 'local',
	};
...
describe('SecurityAudit', () => {
	it('拦截路径遍历', () => {
		const result = new SecurityAudit().audit({
			call_id: 'c1', tool: 'fs.read_file', args: { path: '../secret' }, site: 'local',
		}, new ReadTool(), CTX);
	...
	it('敏感路径仅警告，由工具结果治理负责脱敏', () => {
		const result = new SecurityAudit().audit({
			call_id: 'c3', tool: 'fs.read_file', args: { path: '.env' }, site: 'local',
		}, new ReadTool(), CTX);
		assert.strictEqual(result.allowed, true);
