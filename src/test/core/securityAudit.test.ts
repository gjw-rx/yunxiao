...
class ReadTool extends BaseTool {
	readonly schema: ToolSchema = {
		name: 'fs_read_file', description: 'read', parameters: {}, permissions: 'read',
	};
...
describe('SecurityAudit', () => {
	it('拦截路径遍历', () => {
		const result = new SecurityAudit().audit({
			call_id: 'c1', tool: 'fs_read_file', args: { path: '../secret' },
		}, new ReadTool(), CTX);
	...
	it('危险终端命令交由工具审批', () => {
		const call: ToolCall = { call_id: 'c2', tool: 'terminal_exec', args: { command: 'npm test; rm -rf /' } };
	...
	it('敏感路径仅警告，由工具结果治理负责脱敏', () => {
		const result = new SecurityAudit().audit({
			call_id: 'c3', tool: 'fs_read_file', args: { path: '.env' },
		}, new ReadTool(), CTX);
		assert.strictEqual(result.allowed, true);
