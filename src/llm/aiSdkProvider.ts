 ...
 *
 * Provider-specific 选项（reasoning / DeepSeek thinking）全部隔离在本层，
 * AgentLoop 不感知任何 provider 细节。
 */
import { streamText, type JSONValue } from 'ai';
import type { LLMProvider, LLMRequest, LLMEvent, ReasoningEffort } from './types';
import type { ModelConfig } from '../config/modelConfig';
import { createAiSdkModel, toAiSdkTools, OPENAI_COMPATIBLE_PROVIDER_KEY } from './aiSdkModelFactory';
...
export class AISDKProvider implements LLMProvider {
	...
	async *chatCompletion(request: LLMRequest): AsyncGenerator<LLMEvent> {
		...
		logger.log(
			`# [AISDKProvider] 调用模型 — model=${request.model} messages=${request.messages.length} tools=${tools ? Object.keys(tools).length : 0} toolChoice=${request.toolChoice ?? 'auto'} reasoningEffort=${request.reasoningEffort ?? 'default'} deepseekThinking=${reasoning.deepseekThinking}`,
		);
...
/**
 * 按既有 buildReasoningParams 的语义构造 AI SDK reasoning 参数。
