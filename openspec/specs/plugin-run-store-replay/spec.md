...
The plugin SHALL persist a bounded Run snapshot for each session in VS Code workspace state. Each snapshot MUST contain the cloud `run_id`, its session ID, the last contiguous sequence cursor, lifecycle state, workspace roots, and retained protocol events. An explicit session reset MUST remove that session's snapshot.
...
- **THEN** the plugin reads the same Run ID and cursor 5 from workspace state
