...
## Purpose
定义 Claude 生态配置（SKILL）的同步机制：配置来源为 `claude` 时，从用户级 `~/.claude/skills` 与项目级 `.claude/skills` 加载 SKILL 注册进 skill 注册表，供斜杠菜单与 skill 工具使用。仅当配置来源为 `claude` 时生效；来源为 `none` 或 `trae` 时不加载任何 Claude 目录内容。本期仅同步 SKILL，不读取 `commands`、`settings.json`、MCP 配置等其他 Claude 目录内容。
