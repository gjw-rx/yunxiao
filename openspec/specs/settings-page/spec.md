...
设置页 SHALL 使用左侧分类导航和右侧内容区展示“模型”“Skill”“命令”“MCP”“Hooks”“使用情况”六个分类，导航顺序 SHALL 为模型、Skill、命令、MCP、Hooks、使用情况，默认显示“模型”。模型分类 MUST 显示可编辑的模型配置和保存状态；Skill 分类 MUST 显示已加载 Skill 并提供安装操作的状态反馈；命令分类 MUST 提供全局/项目作用域切换、实际目录、命令列表、刷新及创建/编辑/删除入口；MCP 分类 MUST 提供 JSON 配置入口与 MCP Server 列表管理；Hooks 分类 MUST 提供 Hooks 总开关与内置 RTK Hook 的配置、检测和测试反馈；使用情况分类 MUST 通过扩展宿主请求当前工作区真实的本地 token 用量，并提供日、周、月粒度切换、时间段选择、区间总量及按模型明细。使用情况 MUST 明确数据范围为当前工作区本地已记录 token，且 MUST NOT 将其表述为费用或云端账户统计。
...
#### Scenario: 查看 Command 管理内容
- **WHEN** 用户在设置页点击位于 Skill 下方的“命令”分类
- **THEN** 页面请求并显示全局与项目 Command 快照，提供作用域切换、刷新、创建、编辑和删除操作及明确空状态
...
- **THEN** 页面按新粒度和时间段请求快照并替换总量与模型明细，不影响模型、Skill、命令、MCP 或 Hooks 分类的草稿状态
