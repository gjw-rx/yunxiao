# model-tool-exposure-policy Specification

## Purpose
定义本地模型工具的暴露策略：普通模式与 executing 阶段固定暴露 13 个 OpenCode 职责型工具，职责型 facade 不绕过统一执行治理，工具快照在单次 Agent run 内稳定，暴露统计可观测且不泄露敏感数据。
